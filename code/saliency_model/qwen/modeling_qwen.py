...
class QWenModel(QWenPreTrainedModel):
    ...
    def prepare_inputs_embeds_for_multimodal(
        ...
        image_token_indices = torch.where(input_ids == self.config.visual['image_start_id'])[1]
        inputs_embeds = self.wte(input_ids)

        inputs_embeds_before = [inputs_embed[:image_token_indice+1] for inputs_embed, image_token_indice in zip(inputs_embeds, image_token_indices)]
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if image_features is not None:
                inputs_embeds = self.prepare_inputs_embeds_for_multimodal(input_ids,image_features)
                input_shape = inputs_embeds.shape[:-1]
                attention_mask = torch.cat([torch.zeros(image_features.shape[:-1],device=attention_mask.device)[:,None,None,:],attention_mask],dim=-1)
            else:
                inputs_embeds = self.wte(input_ids)
...
class Qwen(QWenPreTrainedModel):
    ...
    def concat_compute_oneloss(self,pooled_logits,pair_num):
        ...
        pooled_logits = (pooled_logits - torch.ones_like(pooled_logits)*1e12) * torch.exp(self.temperature)
        labels = []
