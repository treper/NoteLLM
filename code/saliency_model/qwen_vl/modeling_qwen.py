...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is None and torch.any(input_ids == self.config.visual['image_start_id']) and images is not None:
        ...
        if images is not None:
            kv_seq_len = hidden_states.size()[1] + images.shape[1]
        else:
        ...
        if (
            ...
            and kv_seq_len == hidden_states.size()[1] + images.shape[1] if images is not None else hidden_states.size()[1]
        ...
        if fake_images is not None:
            ...
            for idx, (i, a, b) in enumerate(img_pos):
                multimodal_embed = [hidden_states[i][:a+1],images[idx],hidden_states[i][b:]]
            ...
            image_attention_mask = torch.ones((batch_size,images.shape[1])).bool().to(device)
        ...
        attention_mask = self._prepare_decoder_attention_mask(
            attention_mask, output_shape[:-1], inputs_embeds, past_length
        )
...
class QWenVL(QWenPreTrainedModel):
    ...
    def __init__(self, config, kwargs=None):
        ...
        print(f'reduction dim : {self.dim_reduction}')
        ...
        if kwargs.fix_image_encoder:
            self.transformer.visual.requires_grad_(False)
            self.transformer.visual.attn_pool.requires_grad_(True)
            self.transformer.visual.ln_post.requires_grad_(True)
            self.transformer.visual.proj.requires_grad_(True)
    ...
    def forward(
        ...
        def get_image_pos(input_ids):
            ...
            image_end = image_token_indices + image_length +1
